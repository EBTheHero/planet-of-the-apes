...
script.on_event(prototypes.recipe["novella"].on_crafted_event, function(event)
...
script.on_event(defines.events.on_tower_planted_seed, function(event)
	...
	local itemstack =
		tower.get_inventory(defines.inventory.agricultural_tower_input)[1]
	game.print("plant")
end)
	...
	function(event)
		...
		if event.entity and event.entity.valid then
			...
			local outputinventory =
				event.entity.get_inventory(defines.inventory.crafter_output)
			...
			local input_stack =
				event.entity.get_inventory(defines.inventory.crafter_input)[1]

			if
				input_stack.valid_for_read and input_stack.name == "monkey-embryon"
			then
...
script.on_event(
	...
	function(event)
		...
		if event.entity and event.entity.valid then
			...
			local x = outputinventory[1]
			if not x.valid_for_read or x.name ~= "monkey-embryon" then return end
...
function became_real(thing)
	if thing.name == "monkey-analyzer-combinator" then
		...
		storage.monkey_analyzers[parent[1]].control_behavior_section =
			thing.entity.get_or_create_control_behavior().get_section(1)
	...
	if thing.name == "monkey-analyzer" then
		...
		thing.entity.get_control_behavior().read_contents = false -- Do not send content
	...
	if thing.name == "monkey-breeder" then
		...
		if not storage.monkey_breeders[thing.id] then
			storage.monkey_breeders[thing.id] = {}
		end
		storage.monkey_breeders[thing.id].entity = thing.entity
	end
...
script.on_event(
	...
	function(eventdata)
		...
		if thing.status == "real" then became_real(thing) end

		if thing.status == "void" or thing.status == "destroyed" then
			became_void(thing)
		end
	end
)
...
script.on_event(
	...
	function(eventdata)
		...
		if eventdata.status == "real" then became_real(eventdata) end
	end
)
